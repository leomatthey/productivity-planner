...
# ---------------------------------------------------------------------------
#  Single event card (with edit form for local events)
# ---------------------------------------------------------------------------

def _render_event_card(event, tasks: list) -> None:
    source_icon  = SOURCE_ICONS.get(event.source or "local", "🔵")
    time_label   = _fmt_time(event.start_datetime)
    # Truncate title for narrow column headers
    short_title  = event.title if len(event.title) <= 18 else event.title[:17] + "…"
    header       = f"{source_icon} {time_label} {short_title}"

    expanded_key = f"cal_expanded_{event.id}"
    expanded     = st.session_state.get(expanded_key, False)

    with st.expander(header, expanded=expanded):
        # Info line: type + duration + location
        dur       = _fmt_duration(event.start_datetime, event.end_datetime)
        type_icon = EVENT_TYPE_ICONS.get(event.event_type or "personal", "📌")
        type_lbl  = EVENT_TYPE_LABELS.get(event.event_type or "personal", event.event_type or "")
        parts     = [f"{type_icon} {type_lbl}"]
        if dur:
            parts.append(f"⏱ {dur}")
        if event.location:
        ...
        if event.is_read_only:
            st.info("🔒 Google Calendar event — read-only here")
            return

        # ── Edit form (local events only) ──────────────────────────────────
        with st.form(f"event_edit_{event.id}"):
            ...
            with ec1:
                cur_type_idx = (
                    ...
                    if event.event_type in EVENT_TYPE_OPTIONS
                    else 1
                )
            ...
            with ed1:
                edit_date = st.date_input(
                    ...
                    key=f"edit_evt_date_{event.id}",
                )
            with ed2:
                # Task selector — active tasks + currently linked task
                task_opts: dict = {None: "— none —"}
                for t in tasks:
                    if t.status not in ("done", "cancelled"):
                        task_opts[t.id] = t.title
            ...
            with et1:
                edit_start = st.time_input(
                    ...
                    key=f"edit_evt_start_{event.id}",
                )
            with et2:
                edit_end = st.time_input(
                    "End", value=event.end_datetime.time(),
                    key=f"edit_evt_end_{event.id}",
                )
            ...
            if saved:
                if not edit_title.strip():
                    ...
                    try:
                        new_start_dt = datetime.combine(edit_date, edit_start)
                        new_end_dt   = datetime.combine(edit_date, edit_end)
                        crud.update_event(
                            event.id,
